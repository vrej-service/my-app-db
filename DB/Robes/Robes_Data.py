...
output_folder = "./Robes_Data"
if not os.path.exists(output_folder):
    os.makedirs(output_folder)

# Load the JSON file with the item links
input_filename = "Robes_results.json"
...
for index, item in enumerate(items):
    ...
    if info_box:
        ...
        level_tag = info_box.find("b", text=lambda t: t and "Level Required:" in t)
        ...
        bonuses = []
        bonuses_tag = info_box.find("b", text=lambda t: t and "Bonuses:" in t)
        if bonuses_tag:
        ...
        # --- Extract Sockets ---
        sockets = []
        sockets_tag = info_box.find("b", text=lambda t: t and "Sockets" in t)
        if sockets_tag:
        ...
        extracted["tradeable"] = bool(info_box.find("b", text=lambda t: t and "Tradeable" in t))
        extracted["no_auction"] = bool(info_box.find("b", text=lambda t: t and "No Auction" in t))
    else:
        print(f"Info box not found for item: {item_title}")
    
    # Create an output filename based on the sanitized item title
    safe_title = sanitize_filename(item_title)
    output_filename = os.path.join(output_folder, f"{safe_title}.json")
    with open(output_filename, "w", encoding="utf-8") as outf:
        json.dump(extracted, outf, indent=2, ensure_ascii=False)
    print(f"Extracted info for '{item_title}' saved to '{output_filename}'")
